...
class TestWriteEventSync:
    """_write_event_sync 同步写入测试。"""

    def test_appends_jsonl(self):
        _write_event_sync({"traceId": "s1", "msg": "first"})
        _write_event_sync({"traceId": "s2", "msg": "second"})
...
class TestReadTracesCompat:
    """验证 read_traces 能正确读取多阶段事件日志。"""

    def test_read_after_event_write(self):
        log_trace_event_sync("r1", Phase.ROUTING_INPUT, {"query": "q1"})
        log_trace_event_sync("r2", Phase.ROUTING_INPUT, {"query": "q2"})
...
class TestFindTraceAggregation:
