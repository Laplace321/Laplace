...
| 日期 | 事件 | 备注 |
| :--- | :--- | :--- |
| 2026-06-15 | v0.5.1 全链路埋点与 BI 适配性改造 | trace_id 走 ContextVar 协程级自动传播（ADR-029），告别 state 显式透传；Phase 常量化 + 11 节点全量接入 `@with_trace` 装饰器统一 input/output/error schema；删旧 API `log_chat_trace_async`；新增 `PipelineState.metric_labels`，classify/execute/clarify/generate 各阶段回填维度，异常路径回填 `error_reason`；新增 `server/bi_index.py` SQLite 索引层（ADR-030），final 事件同步 upsert 到 `bi_index.sqlite`，admin 后台聚合从 JSONL 全扫描升级为 SQL group by；CLI `python -m server.bi_index reindex` 支持全量重建；Prometheus 新增 `pipeline_requests_total` / `skill_calls_total` / `node_latency_seconds_bucket` / `clarifications_total` 4 个业务维度指标，单测断言 skill_name label 集合受控于 SKILL_REGISTRY；日志按天轮转 `query_trace.YYYY-MM-DD.jsonl`（北京时间）+ legacy 单文件 fallback + CLI `python -m server.logger cleanup --keep-days 30`；告警关联 trace_id：alerter 维护 `_recent_failure_traces` FIFO buffer（5 个），LLM 调用失败自动 push，Bark/Telegram 通知正文渲染 `/admin/logs?trace_id=xxx` 链接；新增 ADR-029 / ADR-030 / 架构讨论文档；新增 `tests/test_log_rotation.py` 17 个测试 |
