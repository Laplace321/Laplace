...
(ATK * 0.23
  ...
  * Extra攻击加成 * 指令卡Hit倍率 * 随机数)
± 伤害附加与减免 ∓ 被伤害减免与提升 + BusterChain加成
```
...
### 增伤 BUFF 乘区分类（A/B/C/D）

同一乘区内的 BUFF 相加，不同乘区之间相乘。

| 乘区 | 公式位置 | 包含 BUFF | 条件限定 |
|:---|:---|:---|:---|
| **A 类** | `指令卡伤害倍率 * (1 ± 指令卡性能BUFF ∓ 指令卡耐性)` | 蓝/红/绿魔放 + 色卡耐性下降 | 限定色卡类型 |
| **B 类** | `(1 ± 攻击力BUFF ∓ 防御力BUFF - 特防状态BUFF)` | 加攻 / 降防 / 特防 | 无（无差别增伤） |
| **C 类（卡）** | `(1 + 特攻状态加成 ± 暴击威力BUFF * 暴击补正)` | 特攻状态 + 暴击威力 | 特攻限定敌方特性；暴击限定暴击时 |
| **C 类（宝具）** | `(1 + 特攻状态加成 ± 宝具威力BUFF)` | 特攻状态 + 宝具威力 | 特攻限定敌方特性 |
| **D 类** | `宝具特攻倍率`（独立乘区） | 宝具自带特攻倍率 | 限定敌方特性（宝具固有） |

> 详细攻略文档：`server/data/guides/damage_formula.md`
