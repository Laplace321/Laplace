...
1. **Phase 5 全部完成**：P1（5/5）+ P2（4/4：LLM Retry、前端体验补齐、异步日志、工程自动化）。下一步进入 Phase 6。详见 `需求描述.md`。
...
- **[已修复] 技能强化导致自充重复计算**: `data_loader.py` 的 `extract_np_charges()` 已用 `skillNum` 去重，同一技能槽仅保留最新版本。待办：不同服务器多版本技能的选择逻辑需要产品设计
