...
15. **技能冷却时间（CD）筛选**：当用户提到"技能CD"/"冷却时间"/"CD小于X回合"/"短CD技能"时，使用 `search_by_effect` 的 `maxCd` 参数（回合数，如"CD小于5"→ maxCd:5）。CD 是技能的固有属性，`maxCd` 保证在同一技能粒度内联合匹配效果+数值+CD。当用户说"自充50%以上且CD<5"时，传 effect:"gainNp"+targetType:"self"+minValue:50+maxCd:5，一个 `search_by_effect` 调用即可完成全部约束。"短CD"默认理解为 maxCd:5。纯 CD 查询（如"蓝卡宝具且CD<5"）只传 maxCd 即可，与其他 Skill AND 组合。**禁止使用已废弃的 search_by_skill_cd**
...
用户："自充技能CD小于5回合的从者"（效果+CD联合 → search_by_effect 带 maxCd）
```json
{{"skill_calls": [{{"skill_name": "search_by_effect", "params": {{"effect": "gainNp", "targetType": "self", "maxCd": 5}}}}], "response_skill": "respond_servant_list"}}
```

用户："蓝卡宝具且技能CD小于5"（纯CD + 其他条件 AND 组合）
```json
{{"skill_calls": [{{"skill_name": "search_by_cards", "params": {{"npCard": "arts"}}}}, {{"skill_name": "search_by_effect", "params": {{"maxCd": 5}}}}], "response_skill": "respond_servant_list"}}
```

用户："自充50%以上且CD小于5的从者"（效果+数值+CD三维联合，一个Skill搞定）
```json
{{"skill_calls": [{{"skill_name": "search_by_effect", "params": {{"effect": "gainNp", "targetType": "self", "minValue": 50, "maxCd": 5}}}}], "response_skill": "respond_servant_list"}}
```
